...
class Spreadsheet_Excel_Reader {
	...
	// DUMP AN HTML TABLE OF THE ENTIRE XLS DATA
	// =========================================
	function dumpy($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
		$out = "<table class=\"$table_class\" cellspacing=0>";
		if ($col_letters) {
			$out .= "<thead>\n\t<tr>";
			if ($row_numbers) {
			...
			for($col=1;$col<=$this->colcount($sheet);$col++) {
				...
				if(!$this->sheets[$sheet]['cellsInfo'][$row][$col]['dontprint']) {
					...
					if ($val=='') { $val="kosong"; }
						...
						$cek_kolom_mk = substr($this->val($row,$col,$sheet),0,2);
					...
					//masukkan dalam format data, meskipun data kosong
					$dataSaya[$row][$col] = $val;	
					$out .= "<nobr>".nl2br($val)."</nobr>";
					$out .= "</td>";
				}
			}
