...
class Jadwal_ujian extends CI_Controller {
    ...
    public function index_myschedule($id){
        ...
        $cek = $this->db->query("select * from jadwal_ujian where semester='$semester' AND tahun_ajaran='$tahun_ajaran' AND (dosen_pengawas1='$id' OR dosen_pengawas2='$id')")->num_rows();
        $data['cek'] = $this->db->query("select * from jadwal_ujian where semester='$semester' AND tahun_ajaran='$tahun_ajaran' AND dosen_pengawas1='$id'")->num_rows();
        ...
        if($this->form_validation->run() == FALSE){
            ...
            if($cek>0){
                $data['jadwal_ujian'] = $this->Jadwal_ujian_model->tampilMySchedule($id);
